    ...
    <div class="login-box">
      <div class="login-carousel">
        <el-carousel height="544px" arrow="never">
          <!-- <el-carousel-item v-for="item in 1" :key="item"><img :src="'images/carousel/' + item + '.png'" width="455" alt /></el-carousel-item> -->
          <el-carousel-item><img src="./images/login_left.png" width="455" alt/></el-carousel-item>
        </el-carousel>
      </div>
            ...
            <el-form-item prop="verify" v-if="voiceConfig">
              <div class="block">
                ...
                <div class="verifyCodeBox" v-else>
                  ...
                  <el-tooltip class="item" effect="light" content="发送验证码" placement="right">
                    <el-button
                      circle
                      type="primary"
...
import { getPermission } from '@/api/jsondata.js'
    ...
    submitLogin() {
      this.$refs.loginForm.validate(valid => {
        ...
        loginAjax(param, { sessionId: this.sessionId }).then(resp => {
          ...
          setCookie('userId', resp.data.user.id, 1)
          if (this.uuid) {
            this.$router.replace({
              path: '/callRecords',
          ...
          getPermission().then(p => {
            ...
            if (resp.data.user.roleId == 21) {
              window.open('/ccc/#/session?corpid=' + user.organizationId + '&token=' + token)
              return false
            }

            let home = ''
            if (p && p.menu) {
              home = p.menu.find(item => item.startsWith('/'))
            }
            this.$router.push(home || '/data/call')
          })
        })
      })
    },
