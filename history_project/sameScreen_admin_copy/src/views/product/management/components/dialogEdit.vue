<template>
	<div>
		<el-dialog title="编辑产品" :visible.sync="dialogEdit" width="520px" :before-close="handleClose" @close="closeDialog" :close-on-click-modal="false">
...
<script>
import { modifyProduct, queryType, queryMaterial } from '@/api/robot-ss.js';
	...
	watch: {
		rowObj(newObj, oldObj) {
			...
			if(this.formData.coverImg){
				this.coverImg = this.imgShowUrl + this.formData.coverImg
			}
		},
		immediate: true,
		deep: true
	...
	methods: {
		...
		handelConfirm() {
			...
			this.$refs['elForm'].validate(valid => {
				if (!valid) return;
				modifyProduct(reqData).then(res => {
					if (res.code != 1) {
