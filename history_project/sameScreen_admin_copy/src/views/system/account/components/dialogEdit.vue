...
<script>
import { modifyUser, queryAllOrganization } from '@/api/user.js';
export default {
	inheritAttrs: false,
	components: {},
	...
	watch: {
		rowObj(newObj, oldObj) {
			this.formData = this._.cloneDeep(newObj);
	...
	methods: {
		...
		handelConfirm() {
			...
			delete reqData.productUserRelDTOList;
			this.$refs['elForm'].validate(valid => {
				if (!valid) return;
				modifyUser(reqData).then(res => {
					if (res.code == 0) {
						if (this.notifyInstance) {
							this.notifyInstance.close();
						}
