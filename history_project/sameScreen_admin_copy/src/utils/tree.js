...
function parseNodeJson(nodeJson) {
  ...
  nodeJson.forEach(json => {
    let data = {
      ...
      kbId: json.kbId || "",
      sms: json.sms || "",
      intelligenceMatch: json.intelligenceMatch != null ? json.intelligenceMatch : "",
    ...
    data: {
      ...
      operations: [{
        ...
        sayOptions: {
          ...
          intelligenceInputTimeout: undefined
        }
      }]
    }
  }
...
export function selNode(type, parent, params = {}) {
  let id = SEL_PREFIX + uuid();
  let name = params && params.name ? params.name : "用户回答";
  ...
  let _eachNode = function(node, parent) {
    ...
    if (isOperation(node.id)) {
      ...
      jsonData = {
        ...
        kbId: data ? data.kbId : "",
        sms: data ? data.sms : "",
        intelligenceMatch: data ? data.intelligenceMatch : "",
